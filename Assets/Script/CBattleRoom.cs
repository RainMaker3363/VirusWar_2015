...
// 유니티 엔진에서 수행되는 스크립트들과 게임 서버 사이의 중간 역할을 담당하는 스크립트입니다.
// 그렇기 때문에 게임 로직에 대한 코드는 들어가지 않아서, 추후 다른 게임을 개발할때도 크게 수정하지 않고 재사용 할 수 있게 됩니다.
public class CBattleRoom : MonoBehaviour {
    ...
    // 게임 종료 후 메인으로 돌아갈 때 사용하기 위한 MainTitle객체의 래퍼런스
    CMainTitle main_title;

    // 네트워크 데이터 송, 수신을 위한 네트워크 매니저 래퍼런스.
    CNetworkManager network_manager;

    // 게임 상태에 따라 각각 다른 GUI 모습을 구현하기 위해 필요한 상태 변수
    GAME_STATE game_state;

    // OnGUI메소드에서 호출할 델리게이트
    ...
    // 선명하고 에쁘게 표현하기 위해 폰트 대신 이미지로 만들어 사용한다
